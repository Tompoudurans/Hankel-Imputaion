...
@click.option(
    "--epselon", help="determises how close the model is to data", type=float, default=0.1
)
@click.option(
...
def main(batch, file, epselon, double, maxtime):
    """
    read a file and fill using the hankel imputaion method
    """
    data = pd.read_csv(file, skip_blank_lines=False)
    filled = processing(data, batch, epselon, double=double, verbose=True, max_iters=maxtime)
